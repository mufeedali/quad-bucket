...
get_version() {
	...
	if [ -f "$file" ]; then
		local ver
		ver=$(head -n 1 "$file" | sed -n 's/^# VERSION=//p')
		echo "${ver:-none}"
	else
		echo "missing"
	fi
...
status() {
	...
	for entry in "${UNITS[@]}"; do
		local unit_name rel_path source_path target_path repo_ver installed_ver status_msg
...
install() {
	...
	for entry in "${UNITS[@]}"; do
		local unit_name rel_path source_path target_path
		IFS=":" read -r unit_name rel_path <<<"$entry"
		source_path="$REPO_ROOT/$rel_path"
		target_path="$SYSTEMD_USER_DIR/$unit_name"
...
uninstall() {
	for entry in "${UNITS[@]}"; do
		local unit_name rel_path target_path
		IFS=":" read -r unit_name rel_path <<<"$entry"
		target_path="$SYSTEMD_USER_DIR/$unit_name"
